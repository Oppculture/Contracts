...
contract Lottery is Ownable, ILottery {
    ...
    function setMaxPlayers(uint256 maxPlayers_) external onlyOwner {
        maxPlayers = maxPlayers_;
    }
    function setMinPlayers(uint256 minPlayers_) external onlyOwner {
    ...
    function buyTickets(address sender, uint256 amount) private {
        ...
        if (players[sender] == false) {
            require(_numActivePlayers.add(1) <= maxPlayers);
        ...
        if (numActivePlayers == minPlayers) {
            for (uint i = 0; i <= minPlayers; i++)
                IERC20XP(OppcultureXPContract).Mint(listOfPlayers[i], (amount.mul(XPRatio)).div(10**uint256(18)));
        } else if (numActivePlayers > minPlayers)
            IERC20XP(OppcultureXPContract).Mint(sender, (amount.mul(XPRatio)).div(10**uint256(18))); 
    }
